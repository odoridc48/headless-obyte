# Headless wallet for O<sub>byte</sub> network

This is a headless alternative of the [GUI wallet](../../../byteball) for O<sub>byte</sub> network.  It is designed for an always-online deployment on a server.
...
If you want to change any defaults, refer to the documentation of [ocore](../../../ocore), the core O<sub>byte</sub> library `require()`'d from here.  Below are some headless wallet specific settings you might want to change:
...
* `payout_address`: if you give `pay` command over chat interface, the money will be sent to this O<sub>byte</sub> address.
